...
import { X, Search, Clock, CheckCircle2, Car, Calendar } from 'lucide-react';
...
export default function ReservationPanel({
  isOpen, onClose, availableSpots, onReserve, zoneColors,
  ...
  const filteredSpots = useMemo(() => {
    ...
    return availableSpots.filter(s => s.number.toLowerCase().includes(q) || s.zone.toLowerCase().includes(q));
  ...
  const handleReserve = () => {
    ...
    setReservedList(prev => [...prev, {
      spotNumber: spot.number, plate: plateNumber.trim(),
      until: until.toLocaleString('zh-CN', { hour12: false, month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }),
    }]);
    setSelectedSpotId(null); setPlateNumber(''); setDuration(2);
  };

  const handleClose = () => {
    onClose(); setSelectedSpotId(null); setPlateNumber(''); setDuration(2); setSearchQuery('');
  };
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* Desktop: right panel / Mobile: full-width right panel */}
          <motion.div
            className="fixed top-0 right-0 bottom-0 z-[70] w-full sm:w-[420px] shadow-2xl flex flex-col overflow-hidden"
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="搜索空闲车位..." value={searchQuery}
              ...
              {/* Available spots */}
              <div>
                <p className="text-xs font-semibold mb-2" style={{ color: '#4a5568' }}>空闲车位 ({filteredSpots.length})</p>
                <div className="grid grid-cols-4 sm:grid-cols-4 gap-2 max-h-[200px] overflow-y-auto pr-1">
                  {filteredSpots.map(spot => (
                    <button key={spot.id} onClick={() => setSelectedSpotId(spot.id)}
                      className="py-2.5 px-1 rounded-lg text-xs font-bold transition-all border"
                      style={{
                  ...
                  {filteredSpots.length === 0 && (
                    <div className="col-span-4 text-center py-6 text-sm" style={{ color: '#a0aec0' }}>暂无匹配的空闲车位</div>
                  )}
                </div>
              </div>
              ...
              <AnimatePresence>
                {selectedSpot && (
                  <motion.div className="rounded-xl p-4 space-y-3 border"
                    ...
                    <div>
                      <label className="text-xs font-medium mb-1 block" style={{ color: '#4a5568' }}>车牌号</label>
                      <div className="relative">
                        <Car className="absolute left-3 top-1/2 -translate-y-1/2" size={14} style={{ color: '#718096' }} />
                        <input type="text" placeholder="如：沪A12345" value={plateNumber}
                    ...
                    <div>
                      <label className="text-xs font-medium mb-1 block" style={{ color: '#4a5568' }}>预约时长</label>
                    ...
                    <button onClick={handleReserve} disabled={!plateNumber.trim()}
                      className="w-full py-2.5 rounded-lg text-sm font-bold transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                      style={{ backgroundColor: plateNumber.trim() ? '#059669' : '#a0aec0', color: '#fff' }}>
                      确认预约
                    </button>
                  </motion.div>
                )}
              ...
              {reservedList.length > 0 && (
                <div>
                  <p className="text-xs font-semibold mb-2" style={{ color: '#4a5568' }}>本次预约记录</p>
                  <div className="space-y-2">
                    {reservedList.map((r, i) => (
                      <div key={i} className="flex items-center justify-between p-3 rounded-lg border"
                        ...
                        <span className="text-xs" style={{ color: '#d97706' }}>至 {r.until}</span>
                      </div>
                    ))}
                  </div>
                </div>
