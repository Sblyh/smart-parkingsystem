...
export default function ParkingMap({ floor, spots, selectedSpot, onSelectSpot, zoneColors }: ParkingMapProps) {
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent) => {
    ...
    if (e.touches.length === 1) {
      ...
      lastTapRef.current = { x: e.touches[0].clientX, y: e.touches[0].clientY, t: Date.now() };
      pinchStartRef.current = null;
    } else if (e.touches.length === 2) {
      pinchStartRef.current = {
        dist: getTouchDist(e.touches),
        scale,
        cx: getTouchCenter(e.touches).x,
        cy: getTouchCenter(e.touches).y,
      };
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent) => {
    if (e.touches.length === 1 && touchStartRef.current) {
      e.preventDefault();
      const { x, y, ox, oy } = touchStartRef.current;
      setOffset({
        x: ox + e.touches[0].clientX - x,
        y: oy + e.touches[0].clientY - y,
      });
    } else if (e.touches.length === 2 && pinchStartRef.current) {
      e.preventDefault();
      const { dist, scale: startScale } = pinchStartRef.current;
      const newDist = getTouchDist(e.touches);
      const ratio = newDist / dist;
      const newScale = Math.min(Math.max(startScale * ratio, 0.4), 3);

      // Use the CURRENT finger center (follows fingers in real-time)
      const curCenter = getTouchCenter(e.touches);
      const rect = containerRef.current?.getBoundingClientRect();
      const containerCx = rect ? curCenter.x - rect.left : curCenter.x;
      const containerCy = rect ? curCenter.y - rect.top : curCenter.y;
      const scaleRatio = newScale / scale;

      setOffset(prev => ({
        x: containerCx - (containerCx - prev.x) * scaleRatio,
        y: containerCy - (containerCy - prev.y) * scaleRatio,
      }));
      setScale(newScale);
    }
  }, [scale]);
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full overflow-hidden rounded-xl border select-none"
      ...
      style={{ backgroundColor: isGroundFloor ? '#c0b8a8' : '#F3E9DD', borderColor: 'rgba(0,0,0,0.1)', cursor: isPanning ? 'grabbing' : 'grab', touchAction: 'none' }}>
